...
def main(data_dir, checkpoint_file, target_label):
    ...
    if model == None:
        print('please assign a valid model (ex: UNET, SEGRESNET, UNETR, ...)')
        return
    ...
    targetIndex = getTargetLabelIndex(target_label)
    ...
    with torch.no_grad():
        ...
        for test_data in test_loader:
            ...
            if os.path.isfile(save_nifti_filename) == False:
                if volume_size == 128:
                    nib.save(nib.Nifti1Image(volume_array, np.eye(4)), save_nifti_filename)
                elif volume_size > 128:
                    nib.save(nib.Nifti1Image(resample_twice(volume_array), np.eye(4)), save_nifti_filename)
                print(save_nifti_filename)
                
            saveEachChannel = False
            if saveEachChannel == True:
                multi_outputs = model(test_inputs)
                for targetIndex in range(NUM_OF_LABELS):
                    volume_array = np.array(multi_outputs[0][targetIndex].detach().cpu())
                    save_nifti_filename = os.path.join(data_dir, f"{basename}_{targetIndex}.nii.gz")
                    nib.save(nib.Nifti1Image(volume_array, np.eye(4)), save_nifti_filename)
...
if __name__ == "__main__":
    ...
    checkpoint_file = "./1st.pt"
    target_label    = "AllLabels6"
    a,b,c = main(test_folder, checkpoint_file, target_label)
    metrics = {}
    metrics['pytorch'] = {
    'preprocess_time_sec_cpu': a,
    'inference_time_sec_cpu': b,
    'postprocess_time_sec_cpu': c,
    
}
    
    with open('inference_metrics_cpu.json', 'w', encoding='utf-8') as f:
