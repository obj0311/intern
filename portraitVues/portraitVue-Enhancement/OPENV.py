...
def postprocess(output_tensor, save_path):
    ...
    # 2. [-1,1] → [0,255] 범위 변환 -- -1~1 범위보다 약간 벗어남 -1.3, 1.1 이 정도의 값들도 있음 generator에 tanh가 없어서?
    output_np = (output_np * 0.5 + 0.5) * 255  # → [0,255]
...
'''
pt_path = 'E_model.pth'
g_ema_cpu   = FullGenerator(512, 512, 8, 
                        channel_multiplier=2, narrow=1.0, device='cpu').to('cpu').eval()
g_ema_gpu   = FullGenerator(512, 512, 8, 
                        channel_multiplier=2, narrow=1.0, device='cuda:0').to('cuda:0').eval()
ckpt_cpu = torch.load('E_model.pth', map_location='cpu')
ckpt_gpu = torch.load('E_model.pth', map_location='cuda:0')
g_ema_cpu.load_state_dict(ckpt_cpu['g_ema'])
g_ema_gpu.load_state_dict(ckpt_gpu['g_ema'])
pt_model = g_ema_cpu
pt_gpu_model = g_ema_gpu

#pth파일로 불러올떄 사용하는 코드드
''' 

#input image 전처리
a = time.time()
example_input = preprocess("./input.png")
b = time.time()

pre = b-a

# openvino 변환
# pt 모델 변환에 문제가 생겨서 test.py에서 E.model.pth를 s2.pt로 변환했는데 잘 실행되었다. 그냥 다운받은 s2.pt에 문제가 있었나보다.
...
ov.save_model(ov_model, 'pthPE.xml') # .xml + .bin 생성

# openvino 런타임 코어 객체 생성
core = ov.Core()

# 저장된 xml, bin 파일을 읽어서 network 객체로 로드
network = core.read_model(model = 'pthPE.xml', weights = 'pthPE.bin')
print(network)
...
ovsize_bytes = os.path.getsize('P_E.xml')
ovsize_bytes += os.path.getsize('P_E.bin')
...
while(i<3):
    ...
    #print(len(pt_result_a))
    #print(type(pt_result_a))
    i += 1    
...
c = time.time()
postprocess(pt_output, "output_python_cpu.png")
d = time.time()
post_py_cpu = d-c

aa = time.time()
postprocess(pt_gpu_result, "output_python_gpu.png")
bb = time.time()
post_py_gpu = bb-aa
...
metrics['pytorch'] = {
    ...
    'postprocess_time_sec_cpu': round(post_py_cpu, 4),
    
    'preprocess_time_sec_gpu': round(pre, 4),
    'inference_time_sec_gpu': round(pt_gpu_time, 4),
    'postprocess_time_sec_gpu': round(post_py_gpu, 4),
...
# 오차 검증
pt_np = pt_output.squeeze().cpu().detach().numpy()  # (1,3,512,512)
...
#0~1 이 왜 아닌지 분석
print("pt_fsim min/max:", pt_np.min(), pt_np.max())
print("ov_fsim min/max:", ov_np.min(), ov_np.max())
pt_fsim = (pt_np + 1) / 2  
ov_fsim = (ov_np + 1) / 2        
pt_fsim = torch.clamp(pt_fsim, 0, 1)
ov_fsim = torch.clamp(ov_fsim, 0, 1)
