    ...
    def forward(
        ...
        if noise is None:
            ...
            for i in range(self.n_mlp + 1):
                size    = 2 ** (i+2)
                noise_gen   = torch.randn(batch, self.channels[size], size, size, device=styles[0].device) 
                noise.append(noise_gen)
                noise.append(noise_gen)
...
class FullGenerator(nn.Module):
    def __init__(
        ...
        super().__init__()
        channels = {
            4: int(512 * narrow),8: int(512 * narrow),16: int(512 * narrow),32: int(512 * narrow),
    ...
    def forward(self,inputs, inject_index=None,truncation=1,truncation_latent=None,input_is_latent=False):
        noise = []
        ...
        outs    = self.generator([outs], inject_index, truncation, truncation_latent, input_is_latent, noise=noise)
        return outs
...
class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1], narrow=1, device='cpu'):
        ...
        self.final_linear   = nn.Sequential(
            ...
            # EqualLinear(channels[4], channels[4], activation='fused_lrelu', device=device),
